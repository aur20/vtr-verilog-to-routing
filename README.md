...
This works by dividing the graph in **red** (time-driven) and **black** (combinational) nodes. We assume a simplified architecture in which each CLB has a single function: either flipflop or lookup-table (see [vtr_flow/arch/hes](vtr_flow/arch/hes)). We tested using synthetic benchmarks generated by [GNL](https://github.com/xiaokewan/GNL-Generate-Netlist), a script is available as [vtr_flow/scripts/sweep.sh](vtr_flow/scripts/sweep.sh).
