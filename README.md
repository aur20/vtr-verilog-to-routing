...
We try to improve placement, specifically initial placement, by analysing the graph -- to find the critical path -- before placement. Then we will place the most prominent node (which is on critical path and has highest connectivity) in the middle of the architecture. Next adjacent nodes are placed close to important nodes.

This works by dividing the graph in **red** (time-driven) and **black** (combinational) nodes. We assume a simplified architecture in which each CLB has a single function: either flipflop or lookup-table (see [vtr_flow/arch/hes](vtr_flow/arch/hes)).

The final CPD after SA stays roughly the same (SA still better) but we hope to reduce placement time. We tested using synthetic benchmarks generated by [GNL](https://github.com/xiaokewan/GNL-Generate-Netlist), a script is available as [sweep.sh](sweep.sh). The comparison to default VPR can be plotted using [plot.py](plot.py) (requires matplotlib). Secondly, we inspect some histograms for path lengths in [plot2.ipynb](plot2.ipynb): random placement has scattered a lot of different path lengths while our method keeps them as short as possible. I calculate these paths by a simplified Manhatten grid of the FPGA architecture, wire length might increase.
