...
static bool try_stroobandt_placement(const t_pl_macro& pl_macro,
    ...
    int searchspace = 0;
    while (searchspace < 36) { // some random 6x6 field
        VTR_LOG_MARKUS("\t\t\t\tSearching at %d x %d, %d, %d\n", centroid_loc.x, centroid_loc.y, centroid_loc.layer, centroid_loc.sub_tile);
        if (grid_blocks.block_at_location(centroid_loc) == ClusterBlockId::INVALID() &&
            macro_can_be_placed(pl_macro, centroid_loc, /*check_all_legality=*/false, blk_loc_registry)) {
            break;
        }
        ++position;
        centroid_loc.x = std::clamp((*position).first, 0, (int) device_ctx.grid.width() - 1);
        centroid_loc.y = std::clamp((*position).second, 0, (int) device_ctx.grid.height() - 1);
        if (centroid_loc.x == 0 && (centroid_loc.y == 0 || centroid_loc.y == device_ctx.grid.height() - 1))
            centroid_loc.x = 1;
        if (centroid_loc.x == device_ctx.grid.width() - 1 && (centroid_loc.y == 0 || centroid_loc.y == device_ctx.grid.height() - 1))
            centroid_loc.x -= 1;
    }
    ...
    if (!is_loc_legal(centroid_loc, pr, block_type)) {
        ...
        neighbor_legal_loc = find_centroid_neighbor(centroid_loc, block_type, true, blk_loc_registry);
...
static bool place_macro(int macros_max_num_tries,
    ...
    if (!macro_placed) {
        ...
        macro_placed = try_stroobandt_placement(pl_macro, pr, block_type, pad_loc_type, block_scores, blk_loc_registry);
    }
#endif
...
static vtr::vector<ClusterBlockId, t_block_score> assign_block_scores(const PlaceMacros& place_macros) {
    ...
    std::string redname = "clb_red";
    std::string ioname = "io";
    std::list<ClusterBlockId> blacks;
    std::string blackname = "clb_black";
    ...
    for (const auto& blk_id : cluster_ctx.clb_nlist.blocks()) {
        ...
        VTR_LOG_MARKUS("Found some block: %s\t-\t%s", blk_name.c_str(), blk_type->name);
        // if (redname.find(blk_type->name) != std::string::npos || ioname.find(blk_type->name) != std::string::npos)
        if (redname.find(blk_type->name) != std::string::npos || ioname.find(blk_type->name) != std::string::npos || !cluster_ctx.clb_nlist.block_is_combinational(blk_id))
        {
